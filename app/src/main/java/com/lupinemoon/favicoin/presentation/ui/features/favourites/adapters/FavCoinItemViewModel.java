...
public class FavCoinItemViewModel extends BaseViewModel {

    private FavouritesContract.View favouritesView;

    private CoinItem coinItem;

    private ListItemFavCoinBinding listItemFavCoinBinding;
    ...
    FavCoinItemViewModel(FavouritesContract.View view, @NonNull RequestBuilder requestBuilder,
            ListItemFavCoinBinding listItemCoinBinding) {
        // Set the view locally
        favouritesView = ActivityUtils.checkNotNull(view, "view cannot be null!");
        this.requestBuilder = requestBuilder;
        this.listItemFavCoinBinding = listItemCoinBinding;
    }
    ...
    public void onCoinItemClicked() {
        if ((favouritesView.getActivity() != null) && !doubleClick()) {
            favouritesView.getPresenter().showCoinDetailView(coinItem, listItemFavCoinBinding.coinItemImageView,
                    getImageBitmap());
        }
    }
    ...
    public String getRank() {
        return String.format(favouritesView.getActivity().getString(R.string.rank), coinItem.getRank());
    }

    @Bindable
    public String getPriceUsd() {
        return String.format(favouritesView.getActivity().getString(R.string.dollar_format),
    ...
    public String getPercentChange24h() {
        return String.format(favouritesView.getActivity().getString(R.string.percentage_format),
    ...
    public static void loadTickerChangeImage(final ImageView imageView,
            final FavCoinItemViewModel favCoinItemViewModel) {
        imageView.setImageResource(favCoinItemViewModel.getPercentChange24hValue() > 0 ? R.drawable.vd_ticker_up
                : R.drawable.vd_ticker_down);
    }

    @BindingAdapter(value = { "loadCoinImage" })
    public static void loadCoinImage(final ImageView imageView, final FavCoinItemViewModel favCoinItemViewModel) {
        // Reset the coin bitmap
        favCoinItemViewModel.setImageBitmap(null);

        if (!TextUtils.isEmpty(favCoinItemViewModel.getImageUrl())) {
            favCoinItemViewModel.getRequestBuilder().load(ImageUtils.getFullCoinUrl(favCoinItemViewModel.getImageUrl()))
                    .into(new SimpleTarget<Bitmap>(Target.SIZE_ORIGINAL, Target.SIZE_ORIGINAL) {
                        ...
                        public void onResourceReady(@NonNull Bitmap resource,
                            ...
                            favCoinItemViewModel.setImageBitmap(resource);
                        }
                    });
        } else {
            imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
