...
public class DotsView extends View {
    ...
    private void parseAttributes(Context context, AttributeSet attrs) {
        ...
        try {
            COLOR_1 = a.getColor(R.styleable.DotsView_color1, 0xFFFFC107);
            COLOR_2 = a.getColor(R.styleable.DotsView_color2, 0xFFFF9800);
            COLOR_3 = a.getColor(R.styleable.DotsView_color3, 0xFFFF5722);
            COLOR_4 = a.getColor(R.styleable.DotsView_color4, 0xFFF44336);
        } finally {
            a.recycle();
        }
    }
    ...
    private void updateInnerDotsPosition() {
        if (currentProgress < 0.3f) {
            this.currentRadius2 = (float) WidgetUtils.mapValueFromRangeToRange(
                    currentProgress,
                    0,
                    0.3f,
                    0.f,
        ...
        if (currentProgress < 0.2) {
            ...
            this.currentDotSize2 = (float) WidgetUtils.mapValueFromRangeToRange(
                    currentProgress,
                    0.2f,
                    0.5f,
                    maxDotSize,
    ...
    private void updateOuterDotsPosition() {
        if (currentProgress < 0.3f) {
            this.currentRadius1 = (float) WidgetUtils.mapValueFromRangeToRange(
                    currentProgress,
                    0.0f,
                    0.3f,
                    0,
        ...
        if (currentProgress < 0.7) {
            ...
            this.currentDotSize1 = (float) WidgetUtils.mapValueFromRangeToRange(
                    currentProgress,
                    0.7f,
                    1f,
                    maxDotSize,
    ...
    private void updateDotsPaints() {
        if (currentProgress < 0.5f) {
            float progress = (float) WidgetUtils.mapValueFromRangeToRange(currentProgress, 0f, 0.5f, 0, 1f);
            ...
            float progress = (float) WidgetUtils.mapValueFromRangeToRange(currentProgress, 0.5f, 1f, 0, 1f);
    ...
    private void updateDotsAlpha() {
        float progress = (float) WidgetUtils.clamp(currentProgress, 0.6f, 1f);
        int alpha = (int) WidgetUtils.mapValueFromRangeToRange(progress, 0.6f, 1f, 255, 0);
    ...
    public static final Property<DotsView, Float> DOTS_PROGRESS = new Property<DotsView, Float>(
            Float.class,
            "dotsProgress") {
        @Override
